...
class ReasoningIO(ReasoningBase):
    def __call__(self, messages, temperature=0.1, **kwargs):
        """
        Process messages to generate reasoning
        ...
        Returns:
            str: Generated reasoning result
        """
        examples, task_description = self.process_task_description(messages[-1]["content"])
...
class ReasoningCOTSC(ReasoningBase):
    def __call__(self, messages, temperature=0.1, n=5, **kwargs):
        examples, task_description = self.process_task_description(messages[-1]["content"])
        prompt = '''Solve the task step by step. Your instructions must follow the examples.
Here are some examples.
{examples}
...
class ReasoningTOT(ReasoningBase):
    def __call__(self, messages, temperature=0.1, n=3, **kwargs):
        examples, task_description = self.process_task_description(messages[-1]["content"])
        prompt = '''Solve the task step by step. Your instructions must follow the examples.
Here are some examples.
{examples}
